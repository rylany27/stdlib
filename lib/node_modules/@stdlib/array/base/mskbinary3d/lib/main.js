...
function mskbinary3d( arrays, shape, fcn ) {
	...
	S0 = shape[ 2 ]; // Number of columns
	S1 = shape[ 1 ]; // Number of rows
	S2 = shape[ 0 ]; // Depth
	if ( S0 <= 0 || S1 <= 0 || S2 <= 0 ) {
		return;
	}
	...
	z = arrays[ 3 ];
	m = arrays[ 2 ];
	for ( i2 = 0; i2 < S2; i2++ ) { // Iterate over depth
		x0 = x[ i2 ];
		y0 = y[ i2 ];
		z0 = z[ i2 ];
		m0 = m[ i2 ];
		for ( i1 = 0; i1 < S1; i1++ ) { // Iterate over rows
			x1 = x0[ i1 ];
			y1 = y0[ i1 ];
			z1 = z0[ i1 ];
			m1 = m0[ i1 ];
			for ( i0 = 0; i0 < S0; i0++ ) { // Iterate over columns
				if ( m1[ i0 ] === 0 ) {
					z1[ i0 ] = fcn( x1[ i0 ], y1[ i0 ] );
				}
			}
		}
	}
